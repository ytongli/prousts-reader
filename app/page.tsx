...
import { BookOpen, RotateCcw, Braces, X, ChevronLeft, ChevronRight, Upload, List } from 'lucide-react';
import { Input } from '@/components/ui/input';
import { createReader, inside, shouldClearSentenceHighlight, words, type Range, type State } from '@/lib/reader';
...
export default function Home(){
 ...
 const isAnalyzed=state.components.length>0,selection=state.focus??state.selection,page=book?.pages[pageIndex];
 ...
 const hasVisibleExplanations=visibleStructures.length>0||!!state.plainMeaning;
 ...
 return <div className="app-shell epub-reader">
  ...
  <main className="reading-layout">
   ...
   <section className="book-page" aria-label="EPUB reader" onClick={clearHighlightOnEmptySpace}><div className="mobile-contents"><button className="text-link" aria-expanded={tocOpen} onClick={()=>setTocOpen(v=>!v)}><List size={16}/>Contents</button>{tocOpen&&nav}</div>
    ...
    {loading?<div className="book-loading" role="status"><BookOpen size={25}/><p>Opening your book…</p></div>:page&&<>
     ...
     <div className="book-text" ref={sourceRef} onMouseUp={captureSelection} onKeyUp={captureSelection} onTouchEnd={captureSelection}>
      {page.blocks.map(block=><div key={block.sourceId} data-book-block={block.sourceId} className="book-block">
       <p className={`passage book-${block.kind} ${isAnalyzed&&isSelectedBlock(block)?'is-analyzed':''}`}><ReadingPassage state={state} range={block} active={active} showSelection={selectionVisible} onLayer={layer} onLink={followLink}/></p>
      </div>)}
