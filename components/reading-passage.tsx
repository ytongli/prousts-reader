...
import { inside, exampleReferences, words, type Range, type State } from '@/lib/reader';
import { passageParts } from '@/lib/passage';

export function SourceWords({range,state,active,showSelection,onReference,onLink}:{range:Range;state:State;active:Range|null;showSelection:boolean;onReference:(i:number)=>void;onLink?:(href:string)=>void}){
 const sampleReferences=exampleReferences(state);
 const regions=[...(showSelection&&state.selection.end>state.selection.start?[{...state.selection,style:'selected-words'}]:[]),...(showSelection&&state.focus?[{...state.focus,style:'selected-words'}]:[]),...state.components.map(c=>({...c.range,style:c.role})),...state.structures.filter(s=>s.expanded).map(s=>({...s.range,style:'expanded-words'})),...(state.reference?[{...state.reference.source,style:'ref-source'},{...state.reference.target,style:'ref-target'}]:[]),...(active?[{...active,style:'mapped'}]:[])];
 const bounds=new Set([range.start,range.end]);
 for(const r of [...regions,...sampleReferences.map(r=>r.source),...state.formats])for(const n of [r.start,r.end])if(n>range.start&&n<range.end)bounds.add(n);
 const b=[...bounds].sort((a,b)=>a-b);
 return <>{b.slice(0,-1).map((start,i)=>{const part={start,end:b[i+1]};const styles=regions.filter(r=>inside(part,r)).map(r=>r.style).join(' ');const reference=sampleReferences.findIndex(r=>r.source.start===part.start&&r.source.end===part.end);
 ...
 return reference>=0?<button key={start} className="inline-ref" type="button" onClick={()=>onReference(reference)} aria-label={`What does “${words(part,state.text)}” refer to?`}>{text}</button>:<Fragment key={start}>{text}</Fragment>;})}</>;
}
export function ReadingPassage({state,active,onReference,onLayer,range,onLink,showSelection=true}:{state:State;active:Range|null;onReference:(i:number)=>void;onLayer:(id:string,expand:boolean)=>void;range?:Range;onLink?:(href:string)=>void;showSelection?:boolean}){
 ...
 <span className={p.moved?'moved-phrase':''}><SourceWords range={p.range} state={state} active={active} showSelection={showSelection} onReference={onReference} onLink={onLink}/></span>}
